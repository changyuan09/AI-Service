...
from rotate_ang import (
    Nrotation_angle_get_coor_coordinates,
    Srotation_angle_get_coor_coordinates,
)
from line_intersection import line, intersection, par_line_dist, point_in_line


def boundary_regularization_from_polygon(points, epsilon=6):
    ...
    for i in range(correct_points.shape[0] - 1):
        ...
        if cur_para_vetr_idx != next_para_vetr_idx:
            ...
            if (
                point_intersection is not False
                and not np.isnan(point_intersection).any()
            ):
            ...
            if marg < 3:
                ...
                if not np.isnan(point_move).any():
                ...
                if not np.isnan(add_point_1).any():
                    final_points.append(add_point_1)
                if not np.isnan(add_point_2).any():
                    final_points.append(add_point_2)
    if not np.isnan(final_points[0]).any():
