...
# 生成图片保存的路径
CKimg_dir = '/work/dev/osw-ai-server/data/whu_voc/images'
# 生成标注文件保存的路径
CKanno_dir = '/work/dev/osw-ai-server/data/whu_voc/annotations'
...
def main():
    base_dir = '/work/dev/osw-ai-server/data/whu_voc'  # step1 这里是一个新的文件夹，存放转换后的图片和标注
    image_dir = os.path.join(base_dir, 'images')  # 在上述文件夹中生成images，annotations两个子文件夹
    anno_dir = os.path.join(base_dir, 'annotations')
    mkr(image_dir)
    mkr(anno_dir)
    origin_image_dir = '/work/dev/osw-ai-server/data/whu/train'  # step 2原始的coco的图像存放位置
    origin_anno_dir = '/work/dev/osw-ai-server/data/whu/annotation'  # step 3 原始的coco的标注存放位置
